...
/* The options system needs rework. But for now this is the best I can do to get this to a point where it works for us.
The clone/destroy are conceptually bound to a value. Essentially this is "how do I clone the value" and "how do I delete the value".
Right now each module implements clone and destroy which results in lots of code duplication.
*/
...
static void complete_send_item(LIST_ITEM_HANDLE pending_io_list_item, IO_SEND_RESULT io_send_result)
{
    ...
    (void)singlylinkedlist_remove(wsio_instance->pending_io_list, pending_io_list_item);
...
CONCRETE_IO_HANDLE wsio_create(void* io_create_parameters)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Codes_SRS_WSIO_01_070: [ The underlying uws instance shall be created by calling `uws_client_create`. ]*/
            /* Codes_SRS_WSIO_01_071: [ The arguments for `uws_client_create` shall be: ]*/
            ...
            result->uws = uws_client_create(ws_io_config->hostname, ws_io_config->port, ws_io_config->resource_name, ws_io_config->use_ssl, &protocols, 1);
            if (result->uws == NULL)
            {
                /* Codes_SRS_WSIO_01_075: [ If `uws_client_create` fails, then `wsio_create` shall fail and return NULL. ]*/
...
int wsio_open(CONCRETE_IO_HANDLE ws_io, ON_IO_OPEN_COMPLETE on_io_open_complete, void* on_io_open_complete_context, ON_BYTES_RECEIVED on_bytes_received, void* on_bytes_received_context, ON_IO_ERROR on_io_error, void* on_io_error_context)
{
    ...
    else
    {
        ...
        else
        {
            ...
            /* Codes_SRS_WSIO_01_082: [ `wsio_open` shall open the underlying uws instance by calling `uws_client_open` and providing the uws handle created in `wsio_create` as argument. ] */
            if (uws_client_open(wsio_instance->uws, on_underlying_ws_open_complete, wsio_instance, on_underlying_ws_frame_received, wsio_instance, on_underlying_ws_peer_closed, wsio_instance, on_underlying_ws_error, wsio_instance) != 0)
            {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                /* Codes_SRS_WSIO_01_087: [ `wsio_close` shall call `uws_client_close` while passing as argument the IO handle created in `wsio_create`.  ]*/
                if (uws_client_close(wsio_instance->uws, on_underlying_ws_close_complete, wsio_instance) != 0)
                {
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    /* Codes_SRS_WSIO_01_095: [ `wsio_send` shall call `uws_client_send_frame`, passing the `buffer` and `size` arguments as they are: ]*/
                    ...
                    if (uws_client_send_frame(wsio_instance->uws, WS_FRAME_TYPE_BINARY, buffer, size, true, on_underlying_ws_send_frame_complete, new_item) != 0)
                    {
                        (void)singlylinkedlist_remove(wsio_instance->pending_io_list, new_item);
                        free(pending_socket_io);
                        result = __LINE__;
                    }
                    else
...
static void* wsio_clone_option(const char* name, const void* value)
{
    ...
    else
    {
        ...
        {
            /* Codes_SRS_WSIO_01_171: [ `wsio_clone_option` called with `name` being `WSIOOptions` shall clone the options by calling `OptionHandler_Clone`. ]*/
            result = OptionHandler_Clone((OPTIONHANDLER_HANDLE)value);
            if (result == NULL)
            {
                /* Codes_SRS_WSIO_01_172: [ If `OptionHandler_Clone` fails, `wsio_clone_option` shall fail and return NULL. ]*/
                LogError("Cloning the option handler failed");
            }
        }
        else
        {
